...
public class BagOfWordsGenerator {
	...
	public void loadData() throws Exception {
		File file = new File(
				"/home/arun/code/phd/machine-learning/final-senti.dataset");
		File outFile = new File(
				"/home/arun/code/phd/machine-learning/final-senti.dat");

		InstanceList instanceList = new InstanceList(pipe);

		instanceList.addThruPipe(new CsvIterator(new FileReader(file),
				"(\\w+)\\s+(\\w+)\\s+(.*)", 3, 2, 1) // (data, target, name)
														// field indices
				);
		...
		while (reader.ready()) {
			...
			String line2 = line.substring(line.indexOf('\t'), line.length())
					.trim();
			String line3 = line2.substring(line2.indexOf('\t'), line2.length())
					.trim();
	...
	public double weightedClassifiers(Classifier classifier01,
			Classifier classifier02, InstanceList instanceList, double ratio) {
		int correct = 0;
		...
		// System.out.println("Total :["+instanceList.size()+"] and Correct :["+correct+"] and Accuracy :["+accuracy+"]");
	...
	public void testAndPrint(Classifier classifier, InstanceList instanceList) {
		...
		for (Instance instance : instanceList) {
			...
			String goldLabel = instance.getLabeling().toString();
			String predictedLabel = classification.getLabeling().getBestLabel()
					.toString();
			if (goldLabel.equalsIgnoreCase(predictedLabel)) {
				correct++;
			}
		...
		System.out.println("Total :[" + instanceList.size()
				+ "] and Correct :[" + correct + "] and Accuracy :[" + accuracy
				+ "]");
	...
	public void results() {
		...
		InstanceList targetTrainInstanceList = instanceList.subList(10662,
				13578);
		...
		for (double i = 0.0; i <= 1.1; i = i + 0.05) {
			double accuracy = weightedClassifiers(sourceModel, targetModel,
					testTrainInstanceList, i);
	...
	public void weightedStanfordClassifiers(Classifier classifier02,
		...
		for (Instance instance : instanceList) {
			...
			double value = analyzer.getSentimentScore(instance.getSource()
					.toString());
			LabelVector labelVector02 = classification02.getLabelVector();
